 ...
 * Hệ thống tăng cường trí nhớ AI dựa trên các mốc thời gian
 * * Tác giả: SenriYuki
 * Phiên bản: 1.0.0
 * Dịch thuật: Vietnamese Translation
 */
...
// Đã bổ sung từ khóa Tiếng Việt vào regex tìm kiếm
const HORAE_REGEX_RULES = [
    {
        id: 'horae_hide',
        scriptName: 'Horae - Ẩn thẻ trạng thái',
        description: 'Ẩn thẻ <horae>, không hiển thị trong văn bản chính, không gửi cho AI',
        // Regex đã bổ sung từ khóa tiếng Việt: thời gian, địa điểm, bầu không khí, nhân vật, trang phục, vật phẩm, hảo cảm, kế hoạch
        findRegex: '/(?:<horae>[\\s\\S]*?<\\/horae>||(?:^|\\n)(?:time|thời gian|location|địa điểm|vị trí|atmosphere|bầu không khí|không khí|characters|nhân vật|costume|trang phục|item-?!{0,2}|vật phẩm-?!{0,2}|affection|hảo cảm|độ hảo cảm|npc|agenda-?|lịch trình-?|kế hoạch-?):[^\\n]+(?:\\n(?:time|thời gian|location|địa điểm|vị trí|atmosphere|bầu không khí|không khí|characters|nhân vật|costume|trang phục|item-?!{0,2}|vật phẩm-?!{0,2}|affection|hảo cảm|độ hảo cảm|npc|agenda-?|lịch trình-?|kế hoạch-?):[^\\n]+)*)/gim',
        replaceString: '',
        trimStrings: [],
        placement: [2],
        disabled: false,
    ...
    },
    {
        ...
        description: 'Ẩn hiển thị thẻ <horaeevent>, nhưng vẫn gửi cho AI để truy xuất cốt truyện',
...
const DEFAULT_SETTINGS = {
    ...
    sendTimeline: true,    // Gửi dòng thời gian cốt truyện (tắt sẽ không thể tính thời gian tương đối)
    sendCharacters: true,  // Gửi thông tin nhân vật (trang phục, hảo cảm)
...
let selectedItems = new Set();     // Tên các vật phẩm đã chọn
let longPressTimer = null;         // Bộ đếm thời gian nhấn giữ
let agendaMultiSelectMode = false; // Chế độ đa chọn lịch trình (agenda)
let selectedAgendaIndices = new Set(); // Chỉ mục các lịch trình đã chọn
let agendaLongPressTimer = null;   // Bộ đếm thời gian nhấn giữ lịch trình
...
function ensureRegexRules() {
    ...
    for (const rule of HORAE_REGEX_RULES) {
        ...
        if (idx === -1) {
            ...
            // Nếu đã tồn tại thì đồng bộ nội dung regex (tự động sửa khi cập nhật phiên bản), giữ lại trạng thái disabled của người dùng
    ...
    if (injected > 0) {
        ...
        console.log(`[Horae] Đã tự động tiêm ${injected} quy tắc regex đi kèm`);
    }
}
...
/**
 * Khởi tạo chức năng click thanh điều hướng
 */
...
function getChatTables() {
    ...
    // Tương thích ngược: kiểm tra thuộc tính mảng chat
...
/** Thiết lập bảng tùy chỉnh cho cuộc trò chuyện hiện tại */
function setChatTables(tables) {
    ...
    // Snapshot baseData để khôi phục
...
/**
 * Lấy danh sách việc cần làm do người dùng tạo thủ công (lưu trong chat[0])
 */
...
/**
 * Thiết lập danh sách việc cần làm do người dùng tạo (lưu trong chat[0])
 */
...
/**
 * Lấy tất cả việc cần làm (Người dùng + AI ghi nhận), trả về định dạng thống nhất
...
function getAllAgenda() {
    ...
    // 2. AI ghi nhận (lưu trong horae_meta.agenda của từng tin nhắn)
    const context = getContext();
    if (context?.chat) {
        for (let i = 1; i < context.chat.length; i++) {
            ...
            if (meta?.agenda?.length > 0) {
                for (const item of meta.agenda) {
                    // Khử trùng lặp: kiểm tra xem nội dung đã tồn tại chưa
...
/**
 * Xóa mục việc cần làm được chỉ định
 */
...
function updateStatusDisplay() {
    ...
    if (costumesEl) {
        ...
        // Lọc: chỉ giữ lại nhân vật có trong characters_present
        const entries = presentChars.length > 0
            ? allCostumes.filter(([char]) => presentChars.some(p => p === char || char.includes(p) || p.includes(char)))
            : allCostumes;
        if (entries.length === 0) {
            costumesEl.innerHTML = '<div class="horae-empty-hint">Không có ghi chép trang phục nhân vật tại hiện trường</div>';
        } else {
    ...
    if (itemsEl) {
        ...
        if (entries.length === 0) {
            itemsEl.innerHTML = '<div class="horae-empty-hint">Không có theo dõi vật phẩm</div>';
        } else {
...
function updateTimelineDisplay() {
    ...
    if (events.length === 0) {
        ...
        const searchText = searchKeyword ? `có chứa「${searchKeyword}」` : '';
        listEl.innerHTML = `
            <div class="horae-empty-state">
                <i class="fa-regular fa-clock"></i>
                <span>Tạm thời không có ghi nhận sự kiện ${filterText}${searchText}</span>
            </div>
        `;
        return;
    }
    ...
    listEl.innerHTML = events.reverse().map(e => {
        ...
        // Chuyển đổi nhãn cấp độ
        let levelDisplay = e.event?.level;
        let levelClass = '';
        if (e.event?.level === '关键' || e.event?.level === 'Then chốt') {
            levelClass = 'critical';
            levelDisplay = 'Then chốt';
        } else if (e.event?.level === '重要' || e.event?.level === 'Quan trọng') {
            levelClass = 'important';
            levelDisplay = 'Quan trọng';
        }

        const levelBadge = e.event?.level ? `<span class="horae-level-badge ${levelClass}">${levelDisplay}</span>` : '';
        
        // Lịch tiêu chuẩn hiển thị thứ mấy
        ...
        return `
            <div class="horae-timeline-item horae-editable-item ${levelClass}" data-message-id="${e.messageIndex}">
                ...
                <div class="horae-timeline-content">
                    <div class="horae-timeline-summary">${levelBadge}${e.event?.summary || 'Chưa ghi nhận'}</div>
...
function updateAgendaDisplay() {
    ...
    listEl.innerHTML = agenda.map((item, index) => {
        const sourceIcon = item.source === 'ai'
            ? '<i class="fa-solid fa-robot horae-agenda-source-ai" title="AI ghi nhận"></i>'
    ...
    listEl.querySelectorAll('.horae-agenda-item').forEach(el => {
        ...
        if (agendaMultiSelectMode) {
            // Chế độ đa chọn: nhấn để chuyển đổi chọn
            el.addEventListener('click', (e) => {
                e.stopPropagation();
                toggleAgendaSelection(idx);
            });
        } else {
            // Chế độ thường: nhấn để sửa, nhấn giữ để vào đa chọn
            el.addEventListener('click', (e) => {
                e.stopPropagation();
...
async function deleteSelectedAgenda() {
    ...
    // Lấy danh sách agenda đầy đủ hiện tại, xóa theo thứ tự chỉ mục giảm dần
    const agenda = getAllAgenda();
...
/**
 * Mở cửa sổ thêm/sửa việc cần làm
...
function openAgendaEditModal(agendaItem = null) {
    ...
    const title = isEdit ? 'Sửa việc cần làm' : 'Thêm việc cần làm';
            ...
            <div class="horae-modal-content">
                ...
                <div class="horae-modal-body horae-edit-modal-body">
                    ...
                    <div class="horae-edit-field">
                        ...
                        <textarea id="agenda-edit-text" rows="3" placeholder="Nhập việc cần làm, vui lòng ghi rõ thời gian tuyệt đối cho thời gian tương đối, ví dụ: Ellen mời Alice hẹn hò vào tối Valentine (2026/02/14 18:00)">${escapeHtml(currentText)}</textarea>
                    </div>
                </div>
...
function updateCharactersDisplay() {
    ...
    if (presentEl) {
        if (presentChars.length === 0) {
            presentEl.innerHTML = '<div class="horae-empty-hint">Không có bản ghi</div>';
        } else {
    ...
    // Hảo cảm - Hiển thị phân tầng: Nhân vật quan trọng > Nhân vật có mặt > Khác
    ...
    if (affectionEl) {
        ...
        if (entries.length === 0) {
            affectionEl.innerHTML = '<div class="horae-empty-hint">Không có bản ghi độ hảo cảm</div>';
        } else {
            // Xác định có phải nhân vật quan trọng không
    ...
    if (npcEl) {
        ...
        if (entries.length === 0) {
            npcEl.innerHTML = '<div class="horae-empty-hint">Không có bản ghi nhân vật</div>';
        } else {
            // Xác định nhân vật quan trọng (Nhân vật chính của thẻ hoặc được người dùng đánh dấu thủ công)
            ...
            const renderNpc = (name, info, isFavorite, isMainChar = false) => {
                ...
                if (info.age) {
                    ...
                    if (ageResult.changed) {
                        extraTags.push(`<span class="horae-age-calc" title="Gốc:${ageResult.original} (Đã tính trôi qua)">${ageResult.display} tuổi</span>`);
                    } else {
                ...
                // Ánh xạ biểu tượng giới tính (Bổ sung regex tiếng Việt)
                ...
                if (isMainChar) {
                    ...
                    if (/^(nam|male|m|hùng|đực|trống|♂|男|公|雄)$/.test(g)) {
                        genderIcon = 'fa-solid fa-person';
                        genderClass = 'horae-gender-male';
                    } else if (/^(nữ|female|f|cái|mái|♀|女|雌|母)$/.test(g)) {
            ...
            if (favoriteEntries.length > 0) {
                ...
                html += '<div class="horae-npc-section-title"><i class="fa-solid fa-star"></i> NPC đánh dấu sao</div>';
                ...
                btn.addEventListener('click', () => {
                    ...
                    npcEl.querySelectorAll('.horae-npc-item').forEach(item => {
                        if (filter === 'all') {
                            ...
                            let match = false;
                            // Cập nhật logic lọc theo regex tiếng Việt
                            if (filter === 'male') match = /^(nam|male|m|hùng|đực|trống|♂|男|公|雄)$/.test(g);
                            else if (filter === 'female') match = /^(nữ|female|f|cái|mái|♀|女|雌|母)$/.test(g);
                            else if (filter === 'other') match = !(/^(nam|male|m|hùng|đực|trống|♂|男|公|雄)$/.test(g) || /^(nữ|female|f|cái|mái|♀|女|雌|母)$/.test(g));
...
function toggleNpcFavorite(npcName) {
    ...
    if (index > -1) {
        ...
        showToast(`Đã hủy đánh dấu sao ${npcName}`, 'info');
    } else {
        // Thêm đánh dấu sao
...
function updateItemsDisplay() {
    ...
    if (holderFilterEl) {
        ...
        const holderOptions = ['<option value="all">Tất cả</option>'];
    ...
    if (filterValue !== 'all') {
        // Cập nhật logic lọc để hỗ trợ cả tiếng Trung và tiếng Việt
        entries = entries.filter(([name, info]) => {
            if (filterValue === '!!') return info.importance === '!!' || info.importance === '关键' || info.importance === 'Then chốt';
            if (filterValue === '!') return info.importance === '!' || info.importance === '重要' || info.importance === 'Quan trọng';
            return info.importance === filterValue;
        });
    }
    ...
    if (entries.length === 0) {
        let emptyMsg = 'Không có vật phẩm nào đang theo dõi';
        if (filterValue !== 'all' || holderFilter !== 'all' || searchQuery) {
            emptyMsg = 'Không có vật phẩm phù hợp với bộ lọc';
        }
        listEl.innerHTML = `
            <div class="horae-empty-state">
    ...
    listEl.innerHTML = entries.map(([name, info]) => {
        ...
        // Hỗ trợ cả hai định dạng: ""/"!"/"!!" và "Bình thường"/"Quan trọng"/"Then chốt" (bao gồm cả từ khóa cũ tiếng Trung)
        const isCritical = importance === '!!' || importance === '关键' || importance === 'Then chốt';
        const isImportant = importance === '!' || importance === '重要' || importance === 'Quan trọng';
...
function bindEditButtons() {
    document.querySelectorAll('.horae-item-edit-btn').forEach(btn => {
        // Xóa listener cũ (tránh gắn lặp lại)
                ...
                <div class="horae-modal-body horae-edit-modal-body">
                    ...
                    <div class="horae-edit-field">
                        ...
                        <select id="edit-item-importance">
                            <option value="" ${!item.importance || item.importance === '一般' || item.importance === '' || item.importance === 'Bình thường' ? 'selected' : ''}>Bình thường</option>
                            <option value="!" ${item.importance === '!' || item.importance === '重要' || item.importance === 'Quan trọng' ? 'selected' : ''}>Quan trọng !</option>
                            <option value="!!" ${item.importance === '!!' || item.importance === '关键' || item.importance === 'Then chốt' ? 'selected' : ''}>Then chốt !!</option>
                        </select>
                    </div>
                    <div class="horae-edit-field">
                        <label>Mô tả (Chức năng đặc biệt/Nguồn gốc...)</label>
                        <textarea id="edit-item-desc" placeholder="Ví dụ: Alice tặng trong buổi hẹn hò">${item.description || ''}</textarea>
                    </div>
                    <div class="horae-edit-field">
                    ...
                    <div class="horae-edit-field">
                        <label>Vị trí</label>
                        <input type="text" id="edit-item-location" value="${item.location || ''}" placeholder="Ví dụ: Balo, túi áo, trên bàn trà">
                    </div>
                </div>
                <div class="horae-modal-footer">
...
/**
 * Mở cửa sổ chỉnh sửa độ hảo cảm
 */
function openAffectionEditModal(charName) {
    ...
    document.getElementById('edit-modal-save').addEventListener('click', (e) => {
        ...
        // Cập nhật độ hảo cảm trong tất cả tin nhắn (đặt thành giá trị tuyệt đối)
        const chat = horaeManager.getChat();
...
function openNpcEditModal(npcName) {
    ...
    const genderOptions = [
        ...
        { val: 'Nữ', label: 'Nữ' },
        { val: 'Khác', label: 'Khác' }
    ].map(o => `<option value="${o.val}" ${genderVal === o.val || (o.val === 'Nam' && genderVal === '男') || (o.val === 'Nữ' && genderVal === '女') ? 'selected' : ''}>${o.label}</option>`).join('');
                ...
                <div class="horae-modal-body horae-edit-modal-body">
                    ...
                    <div class="horae-edit-field-row">
                        ...
                        <div class="horae-edit-field horae-edit-field-compact">
                            ...
                            <input type="text" id="edit-npc-race" value="${npc.race || ''}" placeholder="Ví dụ: Con người, Elf">
                        </div>
                        <div class="horae-edit-field horae-edit-field-compact">
    ...
    document.getElementById('edit-modal-delete').addEventListener('click', async (e) => {
        e.stopPropagation();
        e.stopImmediatePropagation();
        if (!confirm(`Bạn có chắc chắn muốn xóa nhân vật 「${npcName}」 không?\n\nThao tác này sẽ xóa thông tin nhân vật khỏi tất cả lịch sử trò chuyện và không thể khôi phục.`)) return;
        ...
        for (let i = 0; i < chat.length; i++) {
            ...
            // Đồng thời xóa bản ghi liên quan trong độ hảo cảm
    ...
    document.getElementById('edit-modal-save').addEventListener('click', async (e) => {
        ...
        // Nếu người dùng sửa đổi tuổi thủ công, cập nhật ngày tham chiếu
...
function openEventEditModal(messageId, eventIndex = 0) {
    ...
    if (!meta) {
        showToast('Không tìm thấy metadata của tin nhắn này', 'error');
        return;
    }
                ...
                <div class="horae-modal-body horae-edit-modal-body">
                    <div class="horae-edit-field">
                        ...
                        <select id="edit-event-level">
                            <option value="一般" ${event.level === '一般' || !event.level || event.level === 'Bình thường' ? 'selected' : ''}>Bình thường</option>
                            <option value="重要" ${event.level === '重要' || event.level === 'Quan trọng' ? 'selected' : ''}>Quan trọng</option>
                            <option value="关键" ${event.level === '关键' || event.level === 'Then chốt' ? 'selected' : ''}>Then chốt</option>
                        </select>
                    </div>
                    <div class="horae-edit-field">
    ...
    document.getElementById('edit-modal-save').addEventListener('click', async (e) => {
        ...
        if (chatMeta) {
            ...
            // Nhắc nhở: Tóm tắt trống đồng nghĩa với việc xóa
            if (!newSummary) {
                if (!confirm('Tóm tắt sự kiện bị trống!\n\nSự kiện này sẽ bị xóa sau khi lưu.\n\nBạn có chắc chắn muốn xóa sự kiện này không?')) {
                    return;
                }
            ...
            // Lưu ý: Logic check '重要' và '关键' được giữ lại cho tương thích, có thể thêm || newLevel === 'Quan trọng'... nếu cần, nhưng select value đã trả về từ khóa tiếng Trung để đảm bảo script logic khác hoạt động
            // Tùy chỉnh: Đã thay đổi value của select ở trên thành từ khóa Tiếng Trung để đảm bảo tương thích với logic 'is_important'
            // Nếu muốn đổi hẳn sang tiếng Việt, cần đổi cả logic bên dưới:
            const isImp = newLevel === '重要' || newLevel === '关键' || newLevel === 'Quan trọng' || newLevel === 'Then chốt';
    ...
    document.getElementById('edit-modal-delete').addEventListener('click', (e) => {
        ...
        if (confirm('Bạn có chắc chắn muốn xóa sự kiện này không?\n\n⚠️ Thao tác này không thể hoàn tác!')) {
            ...
            if (chatMeta) {
                ...
                // Xóa sự kiện ở chỉ mục chỉ định
                if (chatMeta.events.length > eventIndex) {
                    chatMeta.events.splice(eventIndex, 1);
                }
...
/** Ngăn chặn sự kiện nổi bọt từ cửa sổ chỉnh sửa */
...
// ============================================
// Tính năng Bảng tùy chỉnh phong cách Excel
// ============================================

let activeContextMenu = null;

/**
 * Hiển thị danh sách bảng tùy chỉnh
 */
    ...
    if (tables.length === 0) {
        listEl.innerHTML = `
            <div class="horae-custom-tables-empty">
                ...
                <div>Chưa có bảng tùy chỉnh</div>
    ...
    listEl.innerHTML = tables.map((table, tableIndex) => {
        ...
        for (let r = 0; r < rows; r++) {
            ...
            for (let c = 0; c < cols; c++) {
                ...
                // Tính toán động chiều rộng ô nhập liệu
                ...
                tableHtml += `<input type="text" value="${escapeHtml(cellValue)}" size="${inputSize}" data-table="${tableIndex}" data-row="${r}" data-col="${c}" placeholder="${isHeader ? 'Tiêu đề' : ''}">`;
                tableHtml += `</${tag}>`;
            }
            tableHtml += '</tr>';
        }
        ...
        return `
            <div class="horae-excel-table-container" data-table-index="${tableIndex}">
                ...
                <div class="horae-table-prompt-row">
                    <input type="text" value="${escapeHtml(table.prompt || '')}" placeholder="Prompt: Hướng dẫn AI cách điền vào bảng này..." data-table-prompt="${tableIndex}">
                </div>
            </div>
        `;
    }).join('');
...
/**
 * Gắn sự kiện cho bảng Excel
 */
function bindExcelTableEvents() {
    // Sự kiện nhập liệu ô - Tự động lưu + điều chỉnh độ rộng
    ...
    // Nhấn giữ tiêu đề hiển thị menu ngữ cảnh
...
/** Hiển thị menu ngữ cảnh của bảng */
...
function showTableContextMenu(e, tableIndex, row, col) {
    ...
    if (isCorner) {
        menuItems = `
            <div class="horae-context-menu-item" data-action="add-row-below"><i class="fa-solid fa-plus"></i> Thêm dòng</div>
        ...
        menuItems = `
            <div class="horae-context-menu-item" data-action="add-row-above"><i class="fa-solid fa-arrow-up"></i> Thêm dòng phía trên</div>
            <div class="horae-context-menu-item" data-action="add-row-below"><i class="fa-solid fa-arrow-down"></i> Thêm dòng phía dưới</div>
            <div class="horae-context-menu-divider"></div>
            <div class="horae-context-menu-item danger" data-action="delete-row"><i class="fa-solid fa-trash-can"></i> Xóa dòng này</div>
        `;
    } else {
        return;
    }
    ...
    // Gắn sự kiện click item menu - Thực hiện hành động xong đóng menu
    ...
    // Gắn sự kiện trễ, tránh kích hoạt ngay lập tức
    setTimeout(() => {
...
function executeTableAction(tableIndex, row, col, action) {
    ...
    switch (action) {
        ...
        case 'delete-row':
            if (oldRows <= 2) {
                showToast('Bảng cần ít nhất 2 dòng', 'warning');
                return;
            }
            table.rows = oldRows - 1;
            for (const [key, val] of Object.entries(oldData)) {
...
/**
 * Gắn sự kiện danh sách vật phẩm (nhấn giữ, click)
 */
function bindItemsEvents() {
    ...
    items.forEach(item => {
        ...
        // Trong chế độ đa chọn, click để chuyển đổi trạng thái chọn
...
function startLongPress(e, itemName) {
    ...
    }, 800); // 800ms kích hoạt nhấn giữ (kéo dài để tránh chạm nhầm)
}
...
/**
 * Thêm bảng điều khiển metadata cho tin nhắn
 */
...
function buildDeletedItemsDisplay(deletedItems) {
    if (!deletedItems || deletedItems.length === 0) {
        return '<div class="horae-empty-hint">Không có vật phẩm tiêu hao</div>';
    }
...
/**
 * Xây dựng các dòng chỉnh sửa việc cần làm
 */
function buildAgendaEditorRows(agenda) {
    ...
    return agenda.map(item => `
        <div class="horae-editor-row horae-agenda-edit-row">
            ...
            <input type="text" class="agenda-text" style="flex:1 1 0;min-width:0;" value="${escapeHtml(item.text || '')}" placeholder="Nội dung (Thời gian tương đối vui lòng ghi rõ ngày tuyệt đối)">
...
function buildPanelContent(messageIndex, meta) {
    ...
    `).join('') || '<div class="horae-empty-hint">Không có thay đổi trang phục</div>';
    ...
    const itemRows = Object.entries(meta.items || {}).map(([name, info]) => {
        return `
            <div class="horae-editor-row horae-item-row">
                ...
                <input type="text" class="item-holder" value="${info.holder || ''}" placeholder="Người giữ">
        ...
        `;
    }).join('') || '<div class="horae-empty-hint">Không có thay đổi vật phẩm</div>';
    
    // Lấy tổng giá trị hảo cảm của tin nhắn trước
    const prevTotals = {};
    const chat = horaeManager.getChat();
    ...
    const affectionRows = Object.entries(meta.affection || {}).map(([key, value]) => {
        // Phân tích giá trị lớp hiện tại
        ...
        `;
    }).join('') || '<div class="horae-empty-hint">Không có thay đổi hảo cảm</div>';
    
    // Tương thích định dạng sự kiện cũ và mới
    const eventsArr = meta.events || (meta.event ? [meta.event] : []);
    const firstEvent = eventsArr[0] || {};
    // Cập nhật: Kiểm tra cả giá trị tiếng Trung và tiếng Việt để hiển thị đúng lựa chọn trong UI
    ...
    // Cập nhật logic select để hỗ trợ cả 2 ngôn ngữ
    return `
        <div class="horae-panel-grid">
            <div class="horae-panel-row">
                ...
                <div class="horae-panel-value">
                    <input type="text" class="horae-input-datetime" placeholder="Ngày Giờ (VD: 2026/2/4 15:00)" value="${(() => {
            ...
            <div class="horae-panel-row full-width">
                ...
                <div class="horae-event-editor">
                    <select class="horae-input-event-level">
                        ...
                        <option value="Bình thường" ${eventLevel === '一般' || eventLevel === 'Bình thường' ? 'selected' : ''}>Bình thường</option>
                        <option value="Quan trọng" ${eventLevel === '重要' || eventLevel === 'Quan trọng' ? 'selected' : ''}>Quan trọng</option>
                        <option value="Then chốt" ${eventLevel === '关键' || eventLevel === 'Then chốt' ? 'selected' : ''}>Then chốt</option>
                    </select>
        ...
        <div class="horae-panel-rescan">
            ...
            <div class="horae-rescan-buttons">
                ...
                <button class="horae-btn-ai-analyze menu_button" title="Sử dụng AI phân tích nội dung (Tốn API)">
...
function bindPanelEvents(panelEl) {
    ...
    // Mở rộng/Thu gọn - Nhấn vào thanh ngang hoặc nút mở rộng
    ...
    panelEl.querySelector('.horae-btn-add-item')?.addEventListener('click', () => {
        ...
        editor.insertAdjacentHTML('beforeend', `
            <div class="horae-editor-row horae-item-row">
                ...
                <input type="text" class="item-holder" placeholder="Người giữ">
    ...
    panelEl.querySelector('.horae-btn-add-agenda-row')?.addEventListener('click', () => {
        ...
        editor.insertAdjacentHTML('beforeend', `
            <div class="horae-editor-row horae-agenda-edit-row">
                ...
                <input type="text" class="agenda-text" style="flex:1 1 0;min-width:0;" value="" placeholder="Nội dung (Thời gian tương đối vui lòng ghi rõ ngày tuyệt đối)">
                <button class="delete-btn"><i class="fa-solid fa-xmark"></i></button>
            </div>
        `);
        bindDeleteButtons(editor);
    ...
    panelEl.querySelector('.horae-btn-quick-scan')?.addEventListener('click', async () => {
        ...
        // Thử phân tích thẻ tiêu chuẩn trước
        ...
        if (parsed) {
            // Lấy metadata hiện có và hợp nhất
...
/** Quét lại meta tin nhắn và cập nhật bảng điều khiển (thay thế hoàn toàn) */
function rescanMessageMeta(messageId, panelEl) {
    ...
    // Lấy nội dung văn bản (bao gồm thẻ horae ẩn)
    ...
    // Nếu không có trong chat hoặc trống, lấy từ DOM
    if (!messageContent) {
        const mesTextEl = messageEl.querySelector('.mes_text');
        if (mesTextEl) {
    ...
    if (parsed) {
        ...
        // Chỉ giữ lại dữ liệu NPC cũ (nếu phân tích mới không có)
        ...
        // Không có thẻ, xóa dữ liệu (giữ NPC)
        const existingMeta = horaeManager.getMessageMeta(messageId);
        const newMeta = createEmptyMeta();
...
function savePanelData(panelEl, messageId) {
    // Lấy meta hiện có, giữ lại dữ liệu không có vùng chỉnh sửa trong panel (như NPC)
    const existingMeta = horaeManager.getMessageMeta(messageId);
    const meta = createEmptyMeta();
    
    // Giữ lại dữ liệu NPC cũ (vì panel không có vùng chỉnh sửa NPC)
    if (existingMeta?.npcs) {
        ...
        if (nameInput) {
            ...
            if (name) {
                ...
                meta.items[name] = {
                    ...
                    importance: existingImportance,  // Giữ lại phân loại túi đồ
    ...
    if (eventLevel && eventSummary) {
        meta.events = [{
            // Cập nhật: Kiểm tra cả giá trị tiếng Trung và tiếng Việt để đảm bảo tương thích ngược và hoạt động với lựa chọn mới
            is_important: eventLevel === '重要' || eventLevel === '关键' || eventLevel === 'Quan trọng' || eventLevel === 'Then chốt',
            level: eventLevel,
    ...
    if (agendaItems.length > 0) {
        ...
        // Không có dòng chỉnh sửa thì giữ lại agenda cũ
        meta.agenda = existingMeta.agenda;
    }
    
    horaeManager.setMessageMeta(messageId, meta);
    
    // Đồng bộ ghi vào thẻ chính văn bản
...
function buildHoraeTagFromMeta(meta) {
    ...
    // npcs（sử dụng định dạng mới：npc:tên|ngoại hình=tính cách@quan hệ~các trường mở rộng）
    if (meta.npcs) {
...
function buildHoraeEventTagFromMeta(meta) {
    ...
    // Cập nhật: Mặc định là 'Bình thường' (tiếng Việt) nếu không có cấp độ
    const lines = events
        .filter(e => e.summary)
...
function initSettingsEvents() {
    ...
    $('#horae-import-table-file').on('change', (e) => {
        ...
        if (file) {
            importTable(file);
            e.target.value = ''; // Xóa để có thể chọn lại cùng một tệp
        }
    });
    renderCustomTablesList();
...
/**
 * Quét lịch sử với hiển thị tiến độ
 */
async function scanHistoryWithProgress() {
    ...
    try {
        const result = await horaeManager.scanAndInjectHistory(
            ...
            null // Không dùng AI phân tích, chỉ phân tích các thẻ có sẵn
        );
        ...
        showToast(`Quét hoàn tất! Đã xử lý ${result.processed} tin, bỏ qua ${result.skipped} tin`, 'success');
        refreshAllDisplays();
        renderCustomTablesList();
    } catch (error) {
...
async function clearAllData() {
    if (!confirm('Bạn có chắc chắn muốn xóa tất cả dữ liệu Horae không? Thao tác này không thể khôi phục!')) {
        return;
    }
...
/** Sử dụng AI để phân tích nội dung tin nhắn */
async function analyzeMessageWithAI(messageContent) {
    const context = getContext();
    
    const userName = context?.name1 || 'Nhân vật chính';
    
    const analysisPrompt = `Vui lòng phân tích văn bản sau, trích xuất thông tin quan trọng và xuất ra theo định dạng được chỉ định. Nguyên tắc cốt lõi: Chỉ trích xuất thông tin được đề cập rõ ràng trong văn bản, không viết các trường không có, nghiêm cấm bịa đặt.
...
item:emojiTên vật phẩm(Số lượng)|Mô tả=Người giữ@Vị trí chính xác (Chỉ vật phẩm mới nhận hoặc có thay đổi)
item!:emojiTên vật phẩm(Số lượng)|Mô tả=Người giữ@Vị trí chính xác (Vật phẩm quan trọng, bắt buộc mô tả)
item!!:emojiTên vật phẩm(Số lượng)|Mô tả=Người giữ@Vị trí chính xác (Vật phẩm then chốt, mô tả phải chi tiết)
item-:Tên vật phẩm (Vật phẩm tiêu hao/mất)
affection:Tên nhân vật=Giá trị hảo cảm (Chỉ NPC đối với ${userName}, cấm ghi ${userName}, cấm chú thích sau số)
npc:Tên|Ngoại hình=Tính cách@Quan hệ với ${userName}~Giới tính:Nam hoặc Nữ~Tuổi:Số~Chủng tộc:Tên chủng tộc~Nghề nghiệp:Tên nghề nghiệp
agenda:Ngày lập|Nội dung công việc (Chỉ ghi khi xuất hiện lời hứa/kế hoạch/phục bút mới, thời gian tương đối phải chú thích ngày tuyệt đối trong ngoặc)
agenda-:Từ khóa nội dung (Ghi khi công việc đã hoàn thành/hết hạn/hủy bỏ, hệ thống tự động xóa công việc khớp)
</horae>
<horaeevent>
event:Mức độ quan trọng|Tóm tắt sự kiện (30-50 chữ, Bình thường/Quan trọng/Then chốt)
</horaeevent>

【Điều kiện kích hoạt】Chỉ xuất các trường tương ứng khi thỏa mãn điều kiện:
· Vật phẩm: Chỉ ghi khi mới nhận, thay đổi số lượng/sở hữu/vị trí, tiêu hao mất. Không thay đổi không ghi. Đồ đơn lẻ không ghi (1). Tiền tố emoji như 🔑🍞.
· NPC: Lần đầu xuất hiện bắt buộc đầy đủ (bao gồm ~Giới tính/Tuổi/Chủng tộc/Nghề nghiệp). Sau đó chỉ ghi trường thay đổi, không thay đổi không ghi.
  Dấu phân cách: | phân tên, = phân ngoại hình và tính cách, @ phân quan hệ, ~ phân trường mở rộng
· Hảo cảm: Lần đầu xác định theo quan hệ (Người lạ 0-20/Người quen 30-50/Bạn bè 50-70), sau đó chỉ ghi khi thay đổi.
· Việc cần làm: Chỉ ghi khi xuất hiện lời hứa/kế hoạch/phục bút mới. Công việc đã hoàn thành/hết hạn dùng agenda-: để xóa.
  Thêm mới: agenda:2026/02/10|Allen mời ${userName} hẹn hò tối Valentine (2026/02/14 18:00)
  Hoàn thành: agenda-:Allen mời ${userName} hẹn hò tối Valentine
...
/**
 * Kích hoạt khi nhận được phản hồi AI
 */
async function onMessageReceived(messageId) {
    ...
    if (hasTag) {
        console.log(`[Horae] Đã phân tích metadata từ tin nhắn #${messageId}`);
    }
...
function onMessageDeleted() {
    ...
    console.log('[Horae] Phát hiện tin nhắn bị xóa, đang xây dựng lại dữ liệu bảng...');
    horaeManager.rebuildTableData();
    getContext().saveChat();
...
/**
 * Kích hoạt khi chỉnh sửa tin nhắn — Phân tích lại tin nhắn đó và xây dựng lại bảng
 */
function onMessageEdited(messageId) {
    ...
    console.log(`[Horae] Phát hiện tin nhắn #${messageId} đã chỉnh sửa, đang phân tích lại...`);
...
/**
 * Kích hoạt khi hiển thị tin nhắn
 */
