/** Horae - Các hàm tiện ích thời gian */
...
/** Ánh xạ số Trung Quốc (Giữ nguyên để hỗ trợ phân tích đầu vào cũ/Hán Việt) */
const CHINESE_NUMS = {
    '零': 0, '〇': 0,
    '一': 1, '二': 2, '三': 3, '四': 4, '五': 5,
...
function extractDayNumber(dateStr) {
    ...
    // Bổ sung regex tiếng Việt: Ngày X
    const arabicMatch = dateStr.match(/(?:第|Day\s*|day\s*|Ngày\s*)(\d+)(?:日|ngày)?/i) ||
                       dateStr.match(/(\d+)(?:日|号|ngày)/);
...
/** Trích xuất định danh tháng từ chuỗi */
function extractMonthIdentifier(dateStr) {
    if (!dateStr) return null;
    
    // Khớp định dạng "X月" hoặc "Tháng X"
    const monthMatch = dateStr.match(/([^\s\d]+月|Tháng\s*[^\s\d]+)/i);
...
export function parseStoryDate(dateStr) {
    ...
    // Làm sạch chú thích thứ trong tuần do AI viết (Hỗ trợ cả Trung/Việt/Anh)
    let cleanStr = dateStr.trim();
    
    // Regex khớp: (日)...(六) hoặc (CN)...(T7) hoặc (Chủ Nhật)...(Thứ Bảy)
    const weekdayRegex = /\(([日一二三四五六]|CN|T[2-7]|Chủ Nhật|Thứ\s*[HaiBaTưNămSáuBảy]+)\)/i;
    const aiWeekdayMatch = cleanStr.match(weekdayRegex);
    cleanStr = cleanStr.replace(/\s*\(([日一二三四五六]|CN|T[2-7]|Chủ Nhật|Thứ\s*[HaiBaTưNămSáuBảy]+)\)\s*/gi, ' ').trim();
    
    // Ngày không hợp lệ xử lý theo lịch kỳ ảo
    ...
    // Định dạng số tiêu chuẩn YYYY/MM/DD
    ...
    // Định dạng: X年M月D日 HOẶC Năm X Tháng M Ngày D
    // Cần kiểm tra định dạng tiếng Việt trước hoặc song song
    const yearCnMatch = cleanStr.match(/(\d+)年\s*(\d{1,2})月(\d{1,2})日?/) || 
                        cleanStr.match(/(?:Năm\s*)(\d+)\s*(?:Tháng\s*)(\d{1,2})\s*(?:Ngày\s*)(\d{1,2})/i);
    ...
    // Định dạng: X月X日 HOẶC Tháng M Ngày D
    const cnMatch = cleanStr.match(/(\d{1,2})月(\d{1,2})日?/) ||
                    cleanStr.match(/(?:Tháng\s*)(\d{1,2})\s*(?:Ngày\s*)(\d{1,2})/i);
    if (cnMatch) {
...
export function calculateRelativeTime(fromDate, toDate) {
    ...
    // Tính toán chính xác định dạng tiêu chuẩn
...
export function formatStoryDate(dateObj, includeWeekday = false) {
    if (!dateObj) return '';
    // Lịch kỳ ảo giữ nguyên chuỗi gốc
    ...
    if (dateObj.year) {
        if (prefix) {
            // Giữ lại tiền tố lịch pháp, chuyển sang định dạng tiếng Việt: Tiền tố Năm Y Tháng M Ngày D
    ...
    if (includeWeekday && dateObj.month && dateObj.day) {
        ...
        // setFullYear tránh năm tự động nhảy
...
export function generateTimeReference(currentDate) {
    ...
    if (current.type === 'fantasy') {
        return {
            current: currentDate,
            type: 'fantasy',
            note: 'Chế độ lịch kỳ ảo, ngày tương đối được plugin tự động tính toán'
        };
    }
...
/** Tính toán chênh lệch chi tiết giữa hai ngày */
...
/** Trừ đi số ngày nhất định từ ngày hiện tại */
...
export function getTimeOfDay(timeStr) {
    ...
    // Bổ sung regex tiếng Việt
    const matchCN = timeStr.match(/(凌晨|早上|上午|中午|下午|傍晚|晚上|深夜|Rạng sáng|Buổi sáng|Buổi trưa|Buổi chiều|Chập tối|Buổi tối|Đêm khuya)/i);
    if (matchCN) {
        return matchCN[1];
    }
    ...
    if (hour !== null) {
        ...
        if (hour >= 8 && hour < 11) return 'Buổi sáng'; // Sáng muộn
