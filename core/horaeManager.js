 ...
 * Chịu trách nhiệm lưu trữ, phân tích và tổng hợp metadata
 */
 ...
 * @property {string} story_date - Ngày trong cốt truyện, ví dụ "10/1"
 * @property {string} story_time - Giờ trong cốt truyện, ví dụ "15:00" hoặc "Buổi chiều"
 * @property {string} absolute - Dấu thời gian thực tế định dạng ISO
 */
...
/**
 * Trích xuất tên cơ bản của vật phẩm (loại bỏ dấu ngoặc số lượng ở cuối)
 * "Xương bò tươi(5 cân)" → "Xương bò tươi"
 * "Nước sạch(9L)" → "Nước sạch"
 * "Túi cứu thương sơ cấp" → "Túi cứu thương sơ cấp" (Không có số lượng, giữ nguyên)
 * "Túi cứu thương sơ cấp(Đã mở)" → Giữ nguyên (Ngoặc không bắt đầu bằng số không bị loại bỏ)
 */
// Lượng từ cá thể: 1 cái = chỉ một cái, có thể lược bỏ. Lượng từ thuần (cái)(chiếc) cũng vô nghĩa
// Cập nhật lượng từ tiếng Việt, sử dụng dấu | để ngăn cách cho Regex
const COUNTING_CLASSIFIERS = 'cái|chiếc|con|bộ|đôi|tấm|tờ|viên|hạt|cây|bó|ly|chén|dĩa|tô|phần|khẩu|thanh|quả|trái|quyển|cuốn|người|vị|ông|bà|thằng|con|bé|thùng|hộp|chai|lọ|gói|bao|hũ|bình|can|xô|chậu|xe|toà|ngôi|căn';
// Đơn vị chứa/lô: 1 thùng = Một thùng (bên trong có nhiều), không thể lược bỏ
// Đơn vị đo lường (cân/L/kg...): Có ý nghĩa đo lường thực tế, không thể lược bỏ

// ID vật phẩm: 3 chữ số đệm số 0 bên trái, ví dụ 001, 002, ...
function padItemId(id) { return String(id).padStart(3, '0'); }

function getItemBaseName(name) {
    return name
        .replace(/[\(（][\d][\d\.\/]*[a-zA-Z\u4e00-\u9fff\u00C0-\u1EF9]*[\)）]$/, '')  // Số + đơn vị bất kỳ (bao gồm tiếng Việt)
        .replace(new RegExp(`[\\(（](?:${COUNTING_CLASSIFIERS})[\\)）]$`, 'i'), '')  // Lượng từ cá thể thuần túy (AI định dạng sai)
        .trim();
}

/** Tìm vật phẩm đã có theo tên cơ bản */
...
class HoraeManager {
    ...
    /** Thiết lập metadata cho tin nhắn */
    ...
    /** Tổng hợp metadata của tất cả tin nhắn, lấy trạng thái mới nhất */
    getLatestState() {
        const chat = this.getChat();
        ...
        for (let i = 0; i < chat.length; i++) {
            ...
            // Vật phẩm: Hợp nhất cập nhật
            if (meta.items) {
                for (let [name, newInfo] of Object.entries(meta.items)) {
                    // Loại bỏ dấu hiệu số lượng vô nghĩa
                    // (1) Số 1 trần trụi → bỏ
                    name = name.replace(/[\(（]1[\)）]$/, '').trim();
                    // Lượng từ cá thể + số 1 → bỏ (Sử dụng Regex tiếng Việt)
                    name = name.replace(new RegExp(`[\\(（]1(?:${COUNTING_CLASSIFIERS})[\\)）]$`, 'i'), '').trim();
                    // Lượng từ cá thể thuần túy → bỏ
                    name = name.replace(new RegExp(`[\\(（](?:${COUNTING_CLASSIFIERS})[\\)）]$`, 'i'), '').trim();
                    // Đơn vị đo lường/chứa giữ lại
                    ...
                    // Phát hiện dấu hiệu trạng thái tiêu hao, coi như xóa (Regex tiếng Việt)
                    const consumedPatterns = /[\(（](已消耗|已用完|已销毁|消耗殆尽|消耗|用尽|đã tiêu thụ|đã dùng hết|đã hủy|tiêu hao|dùng hết|hết)[\)）]/i;
                    const holderConsumed = /^(消耗|已消耗|已用完|消耗殆尽|用尽|无|tiêu hao|đã tiêu thụ|đã dùng hết|dùng hết|hết|không|vô)$/i;
                    ...
                    // Tên cơ bản khớp với vật phẩm đã có
            ...
            // Hảo cảm: Hỗ trợ giá trị tuyệt đối và tương đối
            if (meta.affection) {
            ...
            if (meta.npcs) {
                // Trường có thể cập nhật vs Trường được bảo vệ
                ...
                for (const [name, newNpc] of Object.entries(meta.npcs)) {
                    ...
                    if (existing) {
                        ...
                        // Khi tuổi thay đổi, ghi lại ngày cốt truyện làm mốc
        ...
        // Phân bổ ID cho vật phẩm chưa có ID
        let maxId = 0;
    ...
    /** Tính toán tuổi hiện tại của NPC dựa trên thời gian trôi qua trong cốt truyện */
    calcCurrentAge(npcInfo, currentStoryDate) {
        const original = npcInfo.age || '';
        const refDate = npcInfo._ageRefDate || '';
        
        // Trường hợp không thể tính toán: Không có tuổi, không có ngày tham chiếu, không có ngày hiện tại
        ...
        if (isNaN(ageNum)) {
            // Tuổi không phải số, không thể tính
            return { display: original, original, changed: false };
        }
        ...
        // Đánh giá ngày tháng để xem đã qua sinh nhật chưa
    ...
    getEvents(limit = 50, filterLevel = 'all') {
        ...
        for (let i = 0; i < chat.length && events.length < limit; i++) {
            ...
            // Hỗ trợ định dạng mới (mảng events) và cũ (event đơn)
            ...
            for (let j = 0; j < metaEvents.length; j++) {
                ...
                events.push({
                    messageIndex: i,
                    eventIndex: j,  // Chỉ mục của sự kiện trong tin nhắn đó
    ...
    /** Lấy danh sách sự kiện quan trọng (tương thích gọi cũ) */
    getImportantEvents(limit = 50) {
        return this.getEvents(limit, 'all');
    }

    /** Tạo nội dung tiêm context rút gọn */
    generateCompactPrompt() {
        const state = this.getLatestState();
        const lines = [];
        
        // Tiêu đề bản chụp trạng thái
        lines.push('[Bản chụp trạng thái hiện tại——Đối chiếu với cốt truyện lượt này, chỉ xuất các trường có thay đổi thực sự trong <horae>]');
        ...
        if (state.timestamp.story_date) {
            ...
            if (sendTimeline) {
                ...
                if (timeRef && timeRef.type === 'standard') {
                    ...
                    // Lịch kỳ ảo
                    lines.push(`[Tham chiếu thời gian|Chế độ lịch kỳ ảo, tham khảo dấu hiệu thời gian tương đối trong quỹ đạo cốt truyện]`);
                }
            }
        }
        ...
        if (sendCharacters) {
            ...
            if (npcs.length > 0) {
                ...
                for (const [name, info] of npcs) {
                    ...
                    // Chủ thể: N[ID] Tên｜Ngoại hình=Tính cách@Quan hệ
        ...
        if (sendTimeline) {
            ...
            if (events.length > 0) {
                ...
                // Lọc: Giữ lại tất cả Then chốt/Quan trọng + 30 sự kiện bình thường gần nhất
                const criticalAndImportant = sortedEvents.filter(e => 
                    e.event?.level === '关键' || e.event?.level === '重要' ||
                    e.event?.level === 'Then chốt' || e.event?.level === 'Quan trọng'
                );
                const normalEvents = sortedEvents.filter(e => 
                    e.event?.level === '一般' || e.event?.level === 'Bình thường' || !e.event?.level
                ).slice(-30);  // Chỉ lấy 30 sự kiện bình thường gần nhất
                
                // Hợp nhất và sắp xếp lại
        ...
        for (const table of customTables) {
            ...
            // Chỉ xuất khi có nội dung hoặc có hướng dẫn điền
            ...
            // Hiển thị thông minh: Ẩn cột trống và dòng trống ở cuối
            // 1. Kiểm tra cột có dữ liệu
            const activeCols = [0]; // col 0 = tiêu đề hàng, luôn hiển thị
            const emptyCols = [];   // Cột hoàn toàn trống
            ...
            // 2. Kiểm tra dòng cuối cùng có dữ liệu
            let lastDataRow = 0;
            ...
            // 5. Ghi chú các dòng trống ở cuối bị lược bỏ
            ...
            if (emptyCols.length > 0) {
                ...
                lines.push(`(${emptyColNames.join('、')}：Tạm không có dữ liệu, cấm điền khi sự kiện tương ứng chưa xảy ra)`);
            }
        }
    ...
    getAffectionLevel(value) {
        if (value >= 80) return 'Chí ái';
        if (value >= 60) return 'Thân mật';
        if (value >= 40) return 'Hảo cảm';
        if (value >= 20) return 'Hữu hảo';
    ...
    parseHoraeTag(message) {
        ...
        for (const line of lines) {
            ...
            // time:10/1 15:00 hoặc time:Năm 2931 2/1 20:30
            if (trimmedLine.startsWith('time:')) {
                const timeStr = trimmedLine.substring(5).trim();
                // Tách thời gian đồng hồ HH:MM từ cuối chuỗi
                ...
                if (clockMatch) {
                    ...
                    // Không có giờ đồng hồ, toàn bộ chuỗi là ngày
                    result.timestamp.story_date = timeStr;
                    result.timestamp.story_time = '';
                }
            }
            // location:Tầng 2 quán cà phê
            ...
            // item:🍺Rượu mạch|Mô tả=Chủ nhân@Vị trí
                ...
                if (eqIndex > 0) {
                    ...
                    let description = undefined;  // undefined = Không có trường mô tả, khi gộp không ghi đè mô tả cũ
                    ...
                    // Loại bỏ dấu hiệu số lượng vô nghĩa (Cập nhật Regex tiếng Việt)
                    itemName = itemName.replace(/[\(（]1[\)）]$/, '').trim();
                    itemName = itemName.replace(new RegExp(`[\\(（]1(?:${COUNTING_CLASSIFIERS})[\\)）]$`, 'i'), '').trim();
                    itemName = itemName.replace(new RegExp(`[\\(（](?:${COUNTING_CLASSIFIERS})[\\)）]$`, 'i'), '').trim();
            ...
            // event:Quan trọng|Alice thú nhận bí mật
                ...
                if (parts.length >= 2) {
                    ...
                    if (levelRaw === '关键' || levelRaw.toLowerCase() === 'critical' || levelRaw === 'Then chốt') {
                        level = 'Then chốt';
                    ...
                    result.events.push({
                        is_important: level === 'Quan trọng' || level === 'Then chốt',
                        level: level,
                        summary: summary
                    });
                }
            }
            // affection:Bob=65 hoặc affection:Bob+5 (Tương thích định dạng cũ)
            else if (trimmedLine.startsWith('affection:')) {
                const affStr = trimmedLine.substring(10).trim();
                // Định dạng mới: Tên=Giá trị (Giá trị tuyệt đối)
                ...
                if (absoluteMatch) {
                    ...
                    // Định dạng cũ: Tên+/-Giá trị (Giá trị tương đối, không có dấu =)
            ...
            // npc:Tên|Ngoại hình=Tính cách@Quan hệ~Giới tính:Nam~Tuổi:25~Chủng tộc:Người~Nghề nghiệp:Lính đánh thuê~Bổ sung:xxx
            // Sử dụng ~ để phân tách các trường mở rộng (key:value)
            ...
            // agenda-:Nội dung đã hoàn thành / agenda:Ngày lập|Nội dung
                ...
                if (pipeIdx > 0) {
                    ...
                    // Chấp nhận lỗi: không có ngày phân cách
    ...
    /** Hợp nhất kết quả phân tích vào metadata */
    mergeParsedToMeta(baseMeta, parsed) {
        ...
        // Hỗ trợ định dạng mới (mảng events)
        ...
        // Thêm các việc cần làm do AI ghi nhận
        ...
        // tableUpdates được truyền như một thuộc tính phụ
    ...
    removeCompletedAgenda(deletedTexts) {
        ...
        const isMatch = (agendaText, deleteText) => {
            ...
            // Khớp chính xác hoặc chứa nhau (cho phép AI viết tắt/mở rộng)
    ...
    /** Xử lý phản hồi AI, phân tích thẻ và lưu trữ metadata */
    processAIResponse(messageIndex, messageContent) {
        ...
        if (parsed) {
            ...
            // Không có thẻ, tạo metadata rỗng
     ...
     * Định dạng: Tên|Ngoại hình=Tính cách@Quan hệ~Giới tính:Nam~Tuổi:25~Chủng tộc:Người~Nghề nghiệp:Lính đánh thuê~Bổ sung:xxx
     */
    _parseNpcFields(npcStr) {
        ...
        for (let i = 1; i < tildeParts.length; i++) {
            ...
            // Khớp từ khóa (Hỗ trợ cả tiếng Anh, Trung, Việt)
            if (/^(性别|gender|sex|Giới tính)$/i.test(key)) info.gender = value;
            else if (/^(年龄|age|年纪|Tuổi)$/i.test(key)) info.age = value;
            else if (/^(种族|race|族裔|族群|Chủng tộc)$/i.test(key)) info.race = value;
            else if (/^(职业|job|class|职务|身份|Nghề nghiệp)$/i.test(key)) info.job = value;
     ...
     * Định dạng: Mỗi dòng một ô 1,1:Nội dung Hoặc một dòng nhiều ô phân cách bằng |
     */
        ...
        for (const line of text.split('\n')) {
            ...
            for (const seg of segments) {
                ...
                if (m) {
                    ...
                    // Lọc bỏ ký hiệu trống (Hỗ trợ tiếng Việt: Trống)
                    if (value && !/^[\(\（]?(空|Trống)[\)\）]?$/i.test(value) && !/^[-—]+$/.test(value)) {
                        updates[`${r}-${c}`] = value;
                    }
                }
            }
    ...
    /** Áp dụng cập nhật bảng vào chat[0] */
    applyTableUpdates(tableUpdates) {
        ...
        for (const update of tableUpdates) {
            ...
            // Ghi vào ô, tự động mở rộng, bảo vệ tiêu đề
    ...
    /** Tái tạo dữ liệu bảng (đảm bảo tính nhất quán sau khi xóa/sửa tin nhắn) */
    rebuildTableData() {
        const chat = this.getChat();
        if (!chat || chat.length === 0) return;
        ...
        for (const table of tables) {
            if (table.baseData) {
                ...
                // Không có baseData: Xóa vùng dữ liệu, giữ lại tiêu đề
        ...
        console.log(`[Horae] Dữ liệu bảng đã được tái tạo, đã phát lại đóng góp từ ${totalApplied} tin nhắn`);
    }
    ...
    /** Tạo nội dung bổ sung cho Prompt hệ thống (Toàn bộ bằng Tiếng Việt) */
    generateSystemPromptAddition() {
        const userName = this.context?.name1 || 'Nhân vật chính';
        const charName = this.context?.name2 || 'Nhân vật';
        
        return `
【Hệ thống Ký ức Horae】(Các ví dụ dưới đây chỉ để minh họa, không dùng nguyên văn trong chính văn!)

═══ Nguyên tắc cốt lõi: Điều khiển bởi sự thay đổi ═══
★★★ Trước khi viết thẻ <horae>, hãy phán đoán xem lượt này thông tin nào đã thay đổi thực sự ★★★
  ① Cơ bản bối cảnh (time/location/characters/costume) → Bắt buộc điền mỗi lượt
  ② Tất cả các trường khác → Tuân thủ nghiêm ngặt [Điều kiện kích hoạt], nếu không thay đổi thì hoàn toàn không viết dòng đó
  ③ Nếu NPC/Vật phẩm đã ghi không có thông tin mới → Cấm xuất ra! Lặp lại dữ liệu không đổi = Lãng phí token
  ④ Thay đổi một phần trường → Sử dụng cập nhật gia tăng, chỉ viết phần thay đổi

═══ Định dạng thẻ ═══
Cuối mỗi lần phản hồi viết hai thẻ sau:
<horae>
...
item-:Tên vật phẩm (Xóa khi vật phẩm tiêu hao/mất. Xem quy tắc, chỉ viết khi kích hoạt)
affection:Tên nhân vật=Độ hảo cảm (Chỉ viết khi kích hoạt)
npc:Tên|Ngoại hình=Tính cách@Quan hệ~Trường mở rộng (Chỉ viết khi kích hoạt)
agenda:Ngày|Nội dung (Viết khi có việc cần làm mới)
agenda-:Từ khóa nội dung (Viết khi việc cần làm đã hoàn thành/hết hạn, hệ thống tự động xóa việc khớp)
</horae>
<horaeevent>
event:Mức độ quan trọng|Tóm tắt sự kiện (30-50 chữ, Mức độ: Bình thường/Quan trọng/Then chốt, ghi lại tóm tắt sự kiện để truy xuất cốt truyện)
</horaeevent>

═══ Quy tắc và Điều kiện kích hoạt 【Vật phẩm】 ═══
Tham chiếu số thứ tự (#ID) trong [Danh sách vật phẩm], nghiêm túc quyết định có xuất ra hay không theo điều kiện sau.

【Khi nào viết】(Thỏa mãn bất kỳ điều kiện nào mới xuất)
  ✦ Nhận được vật phẩm mới → item:/item!:/item!!:
  ✦ Số lượng/Sở hữu/Vị trí/Tính chất của vật phẩm đã có thay đổi → item: (Chỉ viết phần thay đổi)
  ✦ Vật phẩm tiêu hao/mất/dùng hết → item-:Tên vật phẩm
【Khi nào không viết】
  ✗ Vật phẩm không có bất kỳ thay đổi nào → Cấm xuất bất kỳ dòng item nào
  ✗ Vật phẩm chỉ được nhắc đến nhưng trạng thái không đổi → Không viết

【Định dạng】
  Mới nhận: item:emojiTên vật phẩm(Số lượng)|Mô tả=Người giữ@Vị trí chính xác (Có thể bỏ qua trường mô tả. Trừ khi vật phẩm có ý nghĩa đặc biệt như quà tặng, kỷ niệm, thì thêm mô tả)
  Mới nhận (Quan trọng): item!:emojiTên vật phẩm(Số lượng)|Mô tả=Người giữ@Vị trí chính xác (Vật phẩm quan trọng, mô tả bắt buộc: Ngoại quan+Chức năng+Nguồn gốc)
  Mới nhận (Then chốt): item!!:emojiTên vật phẩm(Số lượng)|Mô tả=Người giữ@Vị trí chính xác (Đạo cụ then chốt, mô tả phải chi tiết)
  Vật phẩm cũ thay đổi: item:emojiTên vật phẩm(Số lượng mới)=Người giữ mới@Vị trí mới (Chỉ cập nhật phần thay đổi, không viết | thì giữ nguyên mô tả cũ)
  Tiêu hao/Mất: item-:Tên vật phẩm

【Quy tắc cấp trường】
  · Mô tả: Ghi lại thuộc tính bản chất (Ngoại quan/Chức năng/Nguồn gốc), vật phẩm thường có thể bỏ qua, vật phẩm Quan trọng/Then chốt bắt buộc điền lần đầu
    ★ Đặc điểm ngoại hình (Màu sắc, chất liệu, kích thước..., thuận tiện cho việc miêu tả nhất quán về sau)
    ★ Chức năng/Công dụng
    ★ Nguồn gốc (Ai đưa/Làm sao có được)
       - Ví dụ (Nội dung ví dụ chỉ để minh họa, không dùng nguyên văn trong chính văn!):
         - Ví dụ 1: item!:🌹Bó hoa vĩnh cửu|Hoa hồng vĩnh cửu màu đỏ thẫm, bó bằng ruy băng đen, quà Valentine Allen tặng Lily=Lily@Trên bàn học phòng Lily
         - Ví dụ 2: item!:🎫Vé quay thưởng mười lần may mắn|Vé thưởng bằng giấy phát sáng vàng, phúc lợi tân thủ có thể quay 10 lần trong bể thưởng hệ thống=Lilith@Nhẫn không gian
         - Ví dụ 3: item!!:🏧Máy đổi tiền tệ vị diện tự động|Trông giống cây ATM nhỏ, có thể đổi tiền tệ các vị diện theo tỷ giá thời gian thực=Lilith@Quầy quán rượu
  · Số lượng: Đồ đơn lẻ không viết (1)/(1 cái)/(1 chiếc)..., chỉ đơn vị đo lường mới viết ngoặc như (5 cân)(1L)(1 thùng)
  · Vị trí: Phải là địa điểm cố định chính xác
    ❌ Trên đất trước mặt ai đó, chân ai đó, bên cạnh ai đó, sàn nhà, trên bàn
    ✅ Sàn đại sảnh quán rượu, trên quầy bar nhà hàng, bếp trong nhà, trong ba lô, trên bàn phòng Lilith
  · Cấm tính nội thất cố định và cơ sở kiến trúc vào vật phẩm
  · Mượn tạm ≠ Chuyển quyền sở hữu

Ví dụ (Vòng đời rượu mạch):
  Nhận: item:🍺Rượu mạch ủ lâu(50L)|Rượu mạch tìm thấy trong kho tạp vật, vị chua chát=Lilith@Tủ nguyên liệu bếp sau quán rượu
  Đổi lượng: item:🍺Rượu mạch ủ lâu(25L)=Lilith@Tủ nguyên liệu bếp sau quán rượu
  Dùng hết: item-:Rượu mạch ủ lâu

═══ Quy tắc và Điều kiện kích hoạt 【NPC】 ═══
...
【Khi nào viết】(Thỏa mãn bất kỳ điều kiện nào mới xuất dòng npc: của NPC đó)
  ✦ Lần đầu xuất hiện → Định dạng đầy đủ, tất cả các trường + tất cả trường mở rộng ~(Giới tính/Tuổi/Chủng tộc/Nghề nghiệp), không được thiếu
  ...
  ✦ Nhận được thông tin mới về NPC đó (Chiều cao/cân nặng trước đây chưa biết...) → Bổ sung vào trường tương ứng
  ✦ Bản thân trường mở rộng ~ thay đổi (như nghề nghiệp đổi) → Chỉ viết trường ~ thay đổi
【Khi nào không viết】
  ✗ NPC có mặt nhưng không có thông tin mới → Cấm viết dòng npc:
  ✗ NPC tạm rời đi rồi quay lại, thông tin không đổi → Cấm viết lại
  ✗ Muốn dùng từ đồng nghĩa/viết tắt để viết lại mô tả đã có → Nghiêm cấm!
    ...
    ✅ "Cơ bắp phát triển/Đầy sẹo chiến đấu/Trọng thương"→"Cơ bắp phát triển/Đầy sẹo chiến đấu" (Lành thương, xóa trạng thái lỗi thời)
...
【Ví dụ cập nhật gia tăng】(Lấy NPC Wolfgang làm ví dụ)
  ...
  Chỉ bổ sung ngoại hình: npc:Wolfgang|Người sói lông xám bạc/Cao 220cm/Đầy sẹo chiến đấu/Tay trái băng bó
...
(Lưu ý: Các trường và trường mở rộng ~ không thay đổi hoàn toàn không viết! Hệ thống tự động giữ lại dữ liệu cũ!)

【Quy phạm mô tả quan hệ】
  Phải bao gồm tên đối tượng và chính xác: ❌Khách ✅Vị khách mới của ${userName} / ❌Chủ nợ ✅Người giữ giấy nợ của ${userName} / ❌Chủ nhà ✅Chủ nhà của ${userName} / ❌Bạn trai ✅Bạn trai của ${userName} / ❌Ân nhân ✅Người đã cứu mạng ${userName} / ❌Kẻ bắt nạt ✅Người bắt nạt ${userName} / ❌Người thầm thương ✅Người thầm thương ${userName} / ❌Kẻ thù ✅Cha ruột bị ${userName} giết
  Quan hệ phụ thuộc cần viết rõ tên NPC trực thuộc: ✅Chó săn của Ivan; Thú cưng của khách của ${userName} / Bạn gái của Ivan; Khách của ${userName} / Bạn thân của ${userName}; Vợ của Ivan / Cha dượng của ${userName}; Cha của Ivan / Tình nhân của ${userName}; Em trai của Ivan / Bạn thân của ${userName}; Tình nhân của chồng ${userName}; Người thứ ba chen vào quan hệ vợ chồng giữa ${userName} và Ivan

═══ Điều kiện kích hoạt 【Hảo cảm】 ═══
Chỉ ghi lại độ hảo cảm của NPC đối với ${userName} (Cấm ghi ${userName} tự mình). Mỗi người một dòng, cấm thêm chú thích sau con số.

【Khi nào viết】
  ✦ NPC lần đầu xuất hiện → Xác định giá trị ban đầu theo quan hệ (Người lạ 0-20/Người quen 30-50/Bạn bè 50-70/Người yêu 70-90)
  ✦ Tương tác dẫn đến thay đổi thực tế về hảo cảm → affection:Tên=Tổng giá trị mới
【Khi nào không viết】
  ✗ Hảo cảm không thay đổi → Không viết

═══ Điều kiện kích hoạt 【Việc cần làm】 ═══
【Khi nào viết (Thêm mới)】
  ✦ Trong cốt truyện xuất hiện lời hứa/kế hoạch/lịch trình/nhiệm vụ/phục bút mới → agenda:Ngày|Nội dung
  Định dạng: agenda:Ngày lập|Nội dung (Thời gian tương đối phải mở ngoặc chú thích ngày tuyệt đối)
  Ví dụ: agenda:2026/02/10|Allen mời ${userName} hẹn hò tối Valentine (2026/02/14 18:00)
【Khi nào viết (Hoàn thành xóa bỏ)】
  ✦ Việc cần làm đã hoàn thành/đã mất hiệu lực/đã hủy bỏ → agenda-:Từ khóa nội dung
  Định dạng: agenda-:Nội dung việc cần làm (Viết từ khóa nội dung của việc đã hoàn thành để tự động xóa)
  Ví dụ: agenda-:Allen mời ${userName} hẹn hò tối Valentine
【Khi nào không viết】
  ✗ Việc cần làm đã có không thay đổi → Cấm lặp lại việc cần làm đã có mỗi lượt

═══ Quy tắc định dạng thời gian ═══
Cấm dùng các định dạng mơ hồ như "Day 1"/"Ngày thứ X", bắt buộc sử dụng ngày tháng theo lịch cụ thể.
- Hiện đại: Năm/Tháng/Ngày Giờ:Phút (như 2026/2/4 15:00)
- Lịch sử: Ngày tháng niên đại đó (như 1920/3/15 14:00)
- Kỳ ảo/Hư cấu: Lịch theo thế giới quan đó (như Tháng Sương Giá ngày thứ ba Hoàng hôn)
${this.generateCustomTablesPrompt()}
`;
    }

    /** Tạo prompt cho bảng tùy chỉnh */
...
Phía trên có bảng tùy chỉnh của người dùng, hãy điền dữ liệu theo "Yêu cầu điền".
★ Định dạng: Trong thẻ <horaetable:Tên bảng>, mỗi dòng một ô → Hàng,Cột:Nội dung (Tọa độ bắt đầu từ 0, dữ liệu bắt đầu từ 1,1)
★★★ Nguyên tắc cốt lõi: Chỉ ghi lại sự việc thực tế xảy ra trong cốt truyện! ★★★
  - Cột/Hàng được đánh dấu "Tạm không có dữ liệu" hoặc "Sự kiện chưa xảy ra" → Tuyệt đối cấm điền! Để trống chờ sự kiện xảy ra!
  - Nội dung đã có và không thay đổi → Không viết lại
  - Ô trống không có cốt truyện tương ứng → Không điền
  - Cấm xuất các ký tự giữ chỗ như "(Trống)" "-" "Không"
`;
...
`;
    ...
    /** Phân tích cú pháp lỏng lẻo (Không cần thẻ bao quanh) */
    parseLooseFormat(message) {
        const result = {
            ...
            scene: {},
            agenda: [],   // Việc cần làm
            deletedAgenda: []  // Việc đã hoàn thành
        };
        ...
        while ((match = patterns.item.exec(message)) !== null) {
            ...
            if (eqIndex > 0) {
                ...
                let description = undefined;  // undefined = Không có mô tả
                ...
                // Loại bỏ dấu hiệu số lượng vô nghĩa (Regex tiếng Việt)
                itemName = itemName.replace(/[\(（]1[\)）]$/, '').trim();
                itemName = itemName.replace(new RegExp(`[\\(（]1(?:${COUNTING_CLASSIFIERS})[\\)）]$`, 'i'), '').trim();
                itemName = itemName.replace(new RegExp(`[\\(（](?:${COUNTING_CLASSIFIERS})[\\)）]$`, 'i'), '').trim();
        ...
        while ((match = patterns.affection.exec(message)) !== null) {
            ...
            if (absMatch) {
                ...
                // Định dạng giá trị tương đối name+/-số (không có =)
